...
inline EyeShape happy() {
    ...
    s.outerCornerY = 0.25f;   // Raised outer corners (smile)
    s.wedge = 1.0f;           // Wedge mode: narrow toward outer corners (opposite of angry)
    return s;
}
...
inline EyeShape angry() {
    EyeShape s;
    s.height = 0.8f;
    s.topLid = 0.35f;         // Heavy brow effect
    s.innerCornerY = 0.5f;    // Raised inner corners (angry brow)
    s.outerCornerY = -0.3f;   // Lowered outer corners
    s.wedge = 1.0f;           // Wedge mode: pinch inner corners from both edges
    return s;
}
...
inline EyeShape sleepy() {
    ...
    s.outerCornerY = -0.1f;   // Tired droop
    return s;
}
...
inline EyeShape scared() {
    ...
    s.innerCornerY = 0.2f;    // Raised inner corners (worried brow)
    s.outerCornerY = -0.15f;  // Droopy outer corners (fearful)
    return s;
}
...
inline EyeShape grumpy() {
    ...
    s.innerCornerY = 0.35f;   // Angry inner corners
    s.outerCornerY = -0.15f;  // Slight outer droop
    s.offsetY = -0.1f;        // Looking slightly up
    s.wedge = 1.0f;           // Wedge mode: pinch inner corners
    return s;
}
...
inline EyeShape joyful() {
    ...
    s.squash = 0.85f;         // Extra squash for bouncy cartoon feel
    return s;
}
...
inline EyeShape confused(bool isLeftEye) {
    ...
    // Values exaggerated for readability on small screen
    if (isLeftEye) {
        s.innerCornerY = 0.25f;
        s.outerCornerY = -0.2f;
    } else {
        s.innerCornerY = -0.2f;
        s.outerCornerY = 0.25f;
    }
    return s;
}
...
inline EyeShape curious(bool isLeftEye) {
    EyeShape s;
    if (isLeftEye) {
        s.height = 1.2f;           // Left eye noticeably wider
        s.topLid = 0.0f;
        s.innerCornerY = 0.15f;    // Visible tilt (raised brow)
    } else {
        s.height = 0.85f;          // Right eye clearly smaller
        s.topLid = 0.15f;
        s.innerCornerY = -0.1f;    // Slight droop
    }
    return s;
}
...
inline EyeShape mischievous() {
    ...
    s.height = 0.7f;
    s.topLid = 0.3f;
    s.bottomLid = 0.1f;        // Squinty scheming look
...
inline EyeShape bored() {
    EyeShape s;
    s.height = 0.75f;
    s.topLid = 0.4f;           // Heavier lids
    s.bottomLid = 0.05f;       // Slight bottom lid for heavy-eyed look
    s.offsetY = 0.2f;          // Looking slightly down/aside
    s.outerCornerY = -0.15f;   // Visible droop
    return s;
}
