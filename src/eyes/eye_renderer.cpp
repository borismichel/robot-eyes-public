...
void EyeRenderer::renderToBuf(const EyeShape& shape, uint16_t* buffer,
    ...
    // Pre-compute corner offsets for lid angle (same logic as drawRoundedRect)
    float lidInnerOff = shape.innerCornerY * 30.0f;
    float lidOuterOff = shape.outerCornerY * 30.0f;
    if (isLeftEye) {
        float tmp = lidInnerOff;
        lidInnerOff = lidOuterOff;
        lidOuterOff = tmp;
    }

    if (shape.topLid > 0.0f) {
        applyTopLid(buffer, eyeX, eyeWidth, shape.topLid, eyeY, eyeHeight,
                    lidInnerOff, lidOuterOff, shape.wedge);
    }
...
void EyeRenderer::drawRoundedRect(uint16_t* buffer, int16_t x, int16_t y,
                                   ...
                                   bool isLeftEye, float wedgeFactor) {
    ...
    // Range is approximately ±30 pixels for full -1 to 1 range
    //
    ...
    float innerOffset = innerCornerY * 30.0f;
    float outerOffset = outerCornerY * 30.0f;
    if (isLeftEye) {
    ...
    for (int16_t py = 0; py < curBufHeight; py++) {
        for (int16_t px = 0; px < curBufWidth; px++) {
            ...
            // Allow margin for corner offsets (±30px max) and modifications
            if (rx < -35 || rx >= w + 35 || ry < -20 || ry >= h + 20) continue;
            ...
            //------------------------------------------------------------------
            // Apply Corner Y Offset (Parallelogram/Wedge Blend)
            //------------------------------------------------------------------

            // Corner Y offset shifts or squeezes the eye edges based on wedgeFactor:
            //   wedge=0 → parallelogram: both edges shift same direction (sad droop)
            //   wedge=1 → wedge: edges squeeze from both sides (angry pinch)
            float rowYOffset = innerOffset * (1.0f - normalizedY) + outerOffset * normalizedY;

            // Left edge: parallelogram negates offset (shift whole shape), wedge keeps it (squeeze)
            float edgeLeft = rowYOffset * (2.0f * wedgeFactor - 1.0f);
            // Right edge always shifts by -rowYOffset (same in both modes)
            float edgeRight = (float)w - rowYOffset;
            float adjustedRY = (float)ry;

            // Skip if fully squeezed
            if (edgeLeft >= edgeRight) continue;

            // Calculate effective X bounds with curve applied
            float effectiveLeft = leftEdgeOffset + edgeLeft;
            float effectiveRight = (float)w - rightEdgeOffset - rowYOffset;
            ...
            if (hasPinch || hasCurve) {
                ...
                inside = ((float)rx >= effectiveLeft && (float)rx < effectiveRight &&
                ...
                if (inside && r > 0 && pinchAmount < 0.5f) {
                    ...
                    effectiveR = min(effectiveR, adjW / 2.0f);

                    // Position within the modified shape
                    float localX = (float)rx - effectiveLeft;
                ...
                // Simple geometry path: rounded rectangle with corner offsets
                if ((float)rx >= edgeLeft && (float)rx < edgeRight &&
                    adjustedRY >= 0 && adjustedRY < h) {
                    inside = true;

                    // Corner rounding in edge-local coordinates
                    float localX = (float)rx - edgeLeft;
                    float edgeW = edgeRight - edgeLeft;
                    float effR = min((float)r, edgeW / 2.0f);
                    if (effR > 1.0f) {
                        if (localX < effR && adjustedRY < effR) {
                            float dx = effR - localX;
                            float dy = effR - adjustedRY;
                            if (dx * dx + dy * dy > effR * effR) inside = false;
                        } else if (localX >= edgeW - effR && adjustedRY < effR) {
                            float dx = localX - (edgeW - effR);
                            float dy = effR - adjustedRY;
                            if (dx * dx + dy * dy > effR * effR) inside = false;
                        } else if (localX < effR && adjustedRY >= h - effR) {
                            float dx = effR - localX;
                            float dy = adjustedRY - (h - effR);
                            if (dx * dx + dy * dy > effR * effR) inside = false;
                        } else if (localX >= edgeW - effR && adjustedRY >= h - effR) {
                            float dx = localX - (edgeW - effR);
                            float dy = adjustedRY - (h - effR);
                            if (dx * dx + dy * dy > effR * effR) inside = false;
                        }
...
void EyeRenderer::applyTopLid(uint16_t* buffer, int16_t eyeLeft, int16_t eyeWidth,
                               ...
                               float innerOffset, float outerOffset, float wedgeFactor) {
    // Calculate base lid depth in pixels
    int16_t baseLidPixels = (int16_t)(eyeWidth * lidAmount);
    if (baseLidPixels <= 0) return;

    // Iterate per-row so lid depth can follow the parallelogram shear angle
    for (int16_t py = eyeY - 20; py < eyeY + eyeHeight + 20 && py < curBufHeight; py++) {
        if (py < 0) continue;

        // Calculate corner offset at this row (same interpolation as drawRoundedRect)
        float normalizedY = (eyeHeight > 0) ? constrain((float)(py - eyeY) / (float)eyeHeight, 0.0f, 1.0f) : 0.0f;
        float rowOffset = innerOffset * (1.0f - normalizedY) + outerOffset * normalizedY;

        // Lid starts from the actual top edge position:
        //   parallelogram: negated offset (shift whole shape)
        //   wedge: positive offset (squeeze inward)
        int16_t lidStart = eyeLeft + (int16_t)(rowOffset * (2.0f * wedgeFactor - 1.0f));
        int16_t lidEnd = lidStart + baseLidPixels;

        for (int16_t px = lidStart; px < lidEnd && px < curBufWidth; px++) {
            if (px < 0) continue;
...
void EyeRenderer::applyBottomLid(uint16_t* buffer, int16_t eyeRight, int16_t eyeWidth,
    ...
    // Iterate per-row so lid depth can follow the shear angle
    for (int16_t py = eyeY - 20; py < eyeY + eyeHeight + 20 && py < curBufHeight; py++) {
        if (py < 0) continue;
        ...
        // Right edge always at eyeRight - rowOffset (same in both modes)
        int16_t lidEnd = eyeRight - (int16_t)rowOffset;
        int16_t lidStart = lidEnd - baseLidPixels;
