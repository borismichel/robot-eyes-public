...
struct EyeShape {
    ...
    /**
     * Corner offset mode (0.0-1.0)
     * 0.0 = parallelogram (shift): both edges move same direction (droopy sad)
     * 1.0 = wedge (squeeze): edges move opposite directions (pinched angry)
     * Values between blend both behaviors smoothly.
     */
    float wedge;
    ...
    static EyeShape lerp(const EyeShape& a, const EyeShape& b, float t) {
        ...
        result.wedge = a.wedge + (b.wedge - a.wedge) * t;
