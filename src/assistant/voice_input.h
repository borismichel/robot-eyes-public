...
/** Ring buffer size in bytes (holds ~5 seconds of 16kHz mono audio) */
#define VOICE_RING_BUFFER_SIZE (16000 * 2 * 5)
...
public:
    ...
    /**
     * @brief Set callback for captured audio data (bypasses ring buffer)
     * Called directly from captureAudio() with downsampled 16kHz mono PCM.
     */
    using AudioDataCallback = std::function<void(const uint8_t* data, size_t length)>;
    void onAudioData(AudioDataCallback cb) { audioDataCallback = cb; }
...
private:
    ...
    int16_t downsampleBuffer[VOICE_CAPTURE_SAMPLES / 2 + 1];  // Mono input: 512 / 2.756 ≈ 186 max output samples
