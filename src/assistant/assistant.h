...
struct AssistantConfig {
    ...
    char llmApiKey[256];            ///< API key for LLM provider

    // Voice settings (OpenAI for both STT and TTS)
    char openaiVoiceKey[256];       ///< OpenAI API key for Whisper STT and TTS
    VoiceConfig voiceConfig;        ///< Voice selection and speed

    // STT settings
    char sttLanguage[8];            ///< ISO 639-1 language code (e.g. "de"), empty = auto
    ...
    AssistantConfig() {
        ...
        memset(sttLanguage, 0, sizeof(sttLanguage));
...
public:
    ...
    /**
     * @brief Stop listening and process audio (tap again to stop)
     */
    void stopListening();
...
private:
    ...
    // Chime-before-listen: delay mic capture until activation sound finishes
    bool pendingMicStart;
