...
// JSON schema for start_breathing tool
static const char* START_BREATHING_SCHEMA = R"json({
    "type": "object",
    "properties": {
        "exercise": {
            "type": "string",
            "description": "Exercise type: 'box' for box breathing (default) or 'nadi' for Nadi Shodhana (alternate nostril)",
            "enum": ["box", "nadi"]
        }
    }
})json";
...
inline void registerDeviceTools(LLMClient& llm) {
    ...
    llm.addTool(
        "start_breathing",
        "Start a guided breathing exercise. Two types available: "
        "'box' (5s inhale, hold, exhale, hold x3 = 60s) or "
        "'nadi' Nadi Shodhana alternate nostril breathing (4s phases x3 cycles = 72s). "
        "Defaults to the user's selected type. Use when the user seems stressed or asks to relax.",
        START_BREATHING_SCHEMA
    );
...
inline void registerMcpDeviceTools(MCPServer& mcp) {
    ...
    mcp.addTool("start_breathing",
        "Start a guided breathing exercise. Optional 'exercise' param: 'box' (default) or 'nadi' (alternate nostril).",
        START_BREATHING_SCHEMA);
...
struct DeviceToolCallbacks {
    ...
    std::function<void(const char* exercise)> onStartBreathing;
...
inline String executeDeviceTool(const char* toolName, const char* input) {
    ...
    // Start Breathing
        ...
        if (deviceToolCallbacks.onStartBreathing) {
            const char* exercise = doc["exercise"] | "";
            deviceToolCallbacks.onStartBreathing(exercise);
            bool isNadi = (strcmp(exercise, "nadi") == 0);
            result["success"] = true;
            result["exercise"] = isNadi ? "nadi_shodhana" : "box_breathing";
            result["duration_seconds"] = isNadi ? 72 : 60;
        } else {
