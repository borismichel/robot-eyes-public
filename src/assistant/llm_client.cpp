...
LLMResponse LLMClient::addToolResult(const char* toolUseId, const char* result) {
    ...
    if (response.success) {
        // Store response with tool calls if present
        if (!response.toolCalls.empty()) {
            for (const auto& tc : response.toolCalls) {
                addMessage(MessageRole::Assistant, response.text.c_str(),
                          tc.id.c_str(), tc.name.c_str(), tc.input.c_str());
...
void LLMClient::queueToolResult(const char* toolUseId, const char* result) {
    addMessage(MessageRole::Tool, result, toolUseId, nullptr, nullptr);
}
...
String LLMClient::buildClaudeRequest(const char* newUserMessage) {
    ...
    // Add history — merge consecutive assistant tool_use and tool_result messages
    for (size_t i = 0; i < history.size(); i++) {
        ...
        if (msg.role == MessageRole::User) {
            ...
            while (i + 1 < history.size() && history[i + 1].role == MessageRole::Tool) {
                i++;
                JsonObject nextResult = content.add<JsonObject>();
                nextResult["type"] = "tool_result";
                nextResult["tool_use_id"] = history[i].toolUseId;
                nextResult["content"] = history[i].content;
            }
        } else {
            // Assistant message
            JsonObject msgObj = messages.add<JsonObject>();
            ...
            if (!msg.toolName.isEmpty()) {
                // Merge ALL consecutive assistant tool_use messages into one
                JsonArray content = msgObj["content"].to<JsonArray>();
                ...
                while (i + 1 < history.size() &&
                       ...
                       !history[i + 1].toolName.isEmpty()) {
                    i++;
                    JsonObject nextToolUse = content.add<JsonObject>();
                    nextToolUse["type"] = "tool_use";
                    nextToolUse["id"] = history[i].toolUseId;
                    nextToolUse["name"] = history[i].toolName;
                    JsonDocument nextInputDoc;
                    deserializeJson(nextInputDoc, history[i].toolInput);
                    nextToolUse["input"] = nextInputDoc;
                }
            } else {
                msgObj["content"] = msg.content;
            }
    ...
    for (size_t i = 0; i < history.size(); i++) {
        ...
        if (msg.role == MessageRole::User) {
            ...
            if (!msg.toolName.isEmpty()) {
                ...
                // Merge ALL consecutive assistant tool_call messages
                ...
                while (i + 1 < history.size() &&
                       ...
                       !history[i + 1].toolName.isEmpty()) {
                    i++;
                    JsonObject nextTc = toolCalls.add<JsonObject>();
                    nextTc["id"] = history[i].toolUseId;
                    nextTc["type"] = "function";
                    JsonObject nextFunc = nextTc["function"].to<JsonObject>();
                    nextFunc["name"] = history[i].toolName;
                    nextFunc["arguments"] = history[i].toolInput;
                }
            } else {
                msgObj["content"] = msg.content;
            }
...
LLMResponse LLMClient::makeRequest(const String& body) {
    ...
    uint32_t httpElapsed = millis() - httpStart;

    if (httpCode != HTTP_CODE_OK) {
        Serial.printf("[LLM] HTTP %d after %lums\n", httpCode, httpElapsed);
