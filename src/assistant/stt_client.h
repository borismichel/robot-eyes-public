...
/** Maximum audio buffer size (320KB = ~10 seconds at 16kHz mono 16-bit) */
#define STT_MAX_AUDIO_BUFFER (320 * 1024)
...
public:
    ...
    /**
     * @brief Update API key at runtime
     */
    void setApiKey(const char* key) { strncpy(apiKey, key, sizeof(apiKey) - 1); }

    /**
     * @brief Set language hint for Whisper (ISO 639-1, e.g. "de", "en", "fr")
     * Empty string = auto-detect
     */
    void setLanguage(const char* lang) { strncpy(language, lang, sizeof(language) - 1); }
    ...
    /**
     * @brief Cancel recording without transcribing, reset to Idle
     */
    void cancelRecording();

    /**
     * @brief Log audio buffer stats without transcribing
     */
    void logAudioStats();

    /**
     * @brief Save current audio buffer as WAV file (for debugging)
     * @return true if saved successfully
     */
    bool saveAsWav(const char* path);
...
private:
    ...
    // Configuration
    char apiKey[256];
    char language[8];  ///< ISO 639-1 language code (e.g. "de"), empty = auto
    bool initialized;
