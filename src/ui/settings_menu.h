...
// Main menu pages
#define NUM_MAIN_PAGES 4
#define PAGE_POMODORO 0
#define PAGE_MINDFULNESS 1
...
// Mindfulness sub-menu pages
#define MINDFUL_NUM_PAGES 5
#define MINDFUL_PAGE_BREATHE_NOW 0
#define MINDFUL_PAGE_ENABLE 1
#define MINDFUL_PAGE_SOUND 2
#define MINDFUL_PAGE_INTERVAL 3
#define MINDFUL_PAGE_BACK 4
...
public:
    ...
    // Set breathing exercise reference (for mindfulness menu)
     ...
     * Shows AP SSID, password, and IP address clearly
     ...
     * Shows "Configure WiFi" and "Use Offline" button areas
     * Displayed after user connects to the AP
     * @param color RGB565 color for accent text
     */
...
private:
    ...
    BreathingExercise* breathingExercise;  // Reference to breathing exercise (set externally)
    ...
    // Mindfulness sub-menu state
    bool mindfulSubMenuOpen;   // True when in mindfulness sub-menu
    int mindfulSubPage;        // 0-4 within sub-menu
    static const char* mindfulPageLabels[MINDFUL_NUM_PAGES];
    ...
    // Mindfulness sub-menu helpers
    void openMindfulSubMenu();
    void closeMindfulSubMenu();
    void mindfulNextPage();
    void mindfulPrevPage();
    void renderMindfulSubMenu(uint16_t* buffer, int16_t bufW, int16_t bufH);
    bool handleMindfulSubMenuTouch(bool touched, int16_t x, int16_t y);
